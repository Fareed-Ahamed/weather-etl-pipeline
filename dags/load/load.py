...
class LoadClass:
    def push_raw_data_to_s3(self, weather_data):
        try:
            ...
            object_key = f"raw/{year}/{month}/{day}/{hour}/weather_{timestamp}.json"

            s3_client.put_object(Bucket=bucket_name, Key=object_key, Body=json.dumps(weather_data))
            print(f"Raw data pushed to S3: {bucket_name}/{object_key}")
        except Exception as e:
            print(f"Error pushing raw data to S3: {e}")
            raise
    ...
    def insert_raw_data_to_db(self, weather_data):
        try:
            ...
            print(f"Postgres Secret: {postgres_secrets}")
